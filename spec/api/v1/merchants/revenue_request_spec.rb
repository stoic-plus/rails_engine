...
describe 'Merchants - Revenue' do
  it 'returns total revenue for a date - all merchants' do
    ...
    get "/api/v1/merchants/revenue?date=#{days4_ago}"

    expect(response).to be_successful
    json = JSON.parse(response.body)["data"]
    expect(json["attributes"]["message"]).to eq("No results found for that date")
    expect(json["attributes"]["date"]).to eq(days4_ago.to_s)
  end
  ...
  it 'returns total_revenue for a merchant - no date' do
    c = create(:customer)
    @m_2 = create(:merchant)
    item_2 = create(:item, merchant: @m_2)
    days2_ago = 2.days.ago
    ...
    get "/api/v1/merchants/#{@m_2.id}/revenue"

    expect(response).to be_successful

    json = JSON.parse(response.body)["data"]
    expect(json["type"]).to eq("total_revenue")
    expect(json["attributes"]["total_revenue"]).to eq(23)
  end
end
